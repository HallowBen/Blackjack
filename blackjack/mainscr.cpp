...
void mainscr(int first) {
	if (first == 0) { clear(); }
	printf("Welcome in the world of blackjack!\n\nUser: %s\nMoney: %i\nDebt: %i\n\n\n\t1. Start\n\t2. Help\n\t3. Records\n\t4. Log out", user.username, user.coin, user.debt);
	while (1) {
		char men = _getch();
