...
void newuser() {
	...
	while (c != EOF) {
		fscanf(file, "%i\t%s\t\t%s\t%i\t%i", &user.line, &userinf.username, &userinf.password, &userinf.coin, &user.debt);
	...
	if (c == EOF) {
		...
		user.line = lines;
		fprintf(file, "%i\t%s\t\t%s\t%i\t%i\n", user.line, user.username, user.password, user.coin, user.debt);
	}
	fclose(file);
	startingscreen();
}
...
void loggin(int start) {
	...
	system("cls");
	while (c != EOF) {
		fscanf(file, "%i\t%s\t\t%s\t%i\t%i", &user.line, &user.username, &user.password, &user.coin, &user.debt);
		user=encryption(user);

		if (strcmp(userin, user.username) == 0 and strcmp(passwordin, user.password) == 0) {
			{printf("Welcome back %s!\nYour current money: %i", user.username, user.coin); }
			if(user.debt!=0){ printf("\nYour current debt: %i", user.debt); }
			Sleep(1200);
			fclose(file);
			system("cls");
			mainscr(0);
